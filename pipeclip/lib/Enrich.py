...
def clusterEnrich(clip,threshold=0.01):
    ...
    for index in range(len(epsilon)):
        e = epsilon[index]
        s = step[index]
        r_script = os.path.join(script_path, 'ZTNB_tryCatch.R')
        r_args = ['Rscript', r_script, cluster_filename,str(threshold),str(e),str(s)]
        gc.collect()
...
def clusterEnrich_outsource(clip, threshold=0.01):
    ...
    #epsilon = [0.01,0.15,0.1]
    #step = [0.1,0.08,0.05]
    sh_script = os.path.join(script_path, 'runR1.sh')
