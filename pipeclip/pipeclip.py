...
def runPipeClip(infile,control,outputPrefix,matchLength,mismatch,rmdup,fdrEnrichedCluster,clipType,fdrReliableMutation,species):
    ...
    if myClip.testInput():#check input
        ...
        print("Input BAM: %s \nOutput prefix: %s \nMinimum matched length: %d \nMaximum mismatch count: %d \nPCR duplicate removal code: %d \nFDR for enriched clusters: %f \nFDR for reliable mutations: %f" % (infile,outputPrefix,matchLength,mismatch,rmdup,fdrEnrichedCluster,fdrReliableMutation), file=logfile)
        print("There are %d mapped reads in input BAM file. After filtering,%d reads left" % (myClip.originalMapped,myClip.filteredAlignment), file=logfile)
        print("%d out of %d clusters are enriched." % (myClip.sigClusterCount,len(myClip.clusters)), file=logfile)
        print("%d out of %d mutations are reliable." % (myClip.sigMutationCount,myClip.mutationCount), file=logfile)
        print("%d crosslinking site candidates are found, with %d supporting reliable mutations." % (len(list(myClip.crosslinking.keys())),len(myClip.crosslinkingMutations)), file=logfile)
        logfile.close()
